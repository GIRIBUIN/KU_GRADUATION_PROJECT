...
- `ecampus_sync_apply_service.dart`: 선택한 동기화 항목을 TaskRepository에 생성, 갱신, 제외 상태로 반영
- `ecampus_sync_flow_service.dart`: 기존 e-campus Task 조회, 동기화 미리보기, 선택 항목 반영 흐름을 조합
- `notification_schedule_service.dart`: Task와 NotificationSetting을 기준으로 알림 예약 예정 시각 계산
...
- 가져오지 않기로 선택한 e-campus 항목은 `TaskStatus.excluded`로 저장해 다음 동기화에서 다시 신규 항목으로 뜨지 않게 한다.
- 동기화 미리보기는 완료, 삭제, 제외된 e-campus Task까지 포함해 비교한다.
- 알림 계산은 active Task, dueDate가 있는 Task, enabled 알림 설정만 대상으로 한다.
- 잘못된 알림 시간값이나 이미 지난 예약 시각은 예외 없이 알림 대상에서 제외한다.
