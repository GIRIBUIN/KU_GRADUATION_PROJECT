...
- 로그인 입력은 WebView가 담당하고 앱은 크리덴셜을 직접 다루지 않는다.
- 로그아웃은 `EcampusSession`의 쿠키로 e-campus logout endpoint를 호출한 뒤 WebView 쿠키를 삭제한다.
- 세션 쿠키가 없으면 네트워크 logout 요청 없이 로컬 WebView 쿠키만 정리한다.
